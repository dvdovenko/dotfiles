.PHONY: bootstrap darwin-bootstrap darwin-switch darwin-build vps-switch vps-build dotfiles-apply
...
darwin-bootstrap:
	sudo -H $(shell command -v nix) run --inputs-from "path:$(CURDIR)/nix" nix-darwin -- switch --flake "path:$(CURDIR)/nix#danylo-mbp"
...
darwin-build:
	nix build --impure "path:$(CURDIR)/nix#darwinConfigurations.danylo-mbp.system" --no-link
